...
**协调服务器**高可靠地存储着集群配置信息，以及数据分区到存储服务器的位置映射。应用服务器缓存着位置映射，可以直接向存储服务器发出存储请求。

####  副本管理机制及读/写流程
RAMCloud使用基于**日志**的方式存储数据。

对于每个数据分区，Master将其划分为两部分，一是用于追加写入的日志区，另一是散列索引。

在日志区中，键值对被存储，而散列索引用于查询日志区中的键值对。这里的日志区由固定长度（如8MB）的日志段组成，它是副本机制的基本单元。鉴于DRAM的高昂成本和数据丢失风险，RAMCloud将日志段的副本存储在磁盘上。

**基于日志的存储方式在RAMCloud中有两个重要好处：**
...
当 Backup 的本地非易失性缓冲区被写满了，它会将缓冲区中积攒的日志段写入磁盘。RAMCloud 引入非易失性缓冲区是为了同时实现副本操作的低延迟和持久化。
对于读操作，客户端将请求发送至对应的 Master，Master 查询对应数据分区的散列表，定位到日志区中的键值对，并返回至客户端。

### 分布式键值存储系统关键技术
#### 1.分布式副本技术

分布式键值存储系统利用分布式副本技术提供高可用性。
除了Dynamo使用的基于法定人数的副本机制(Quorum-based Replication)和RAMCloud使用的主从备份(Primary-backup Replication)，常用的分布式副本技术还有基于共识协议(Consensus Protocol)的副本机制。

![image](https://github.com/lus-oa/Distributed-Storage/assets/122666739/e93fe6d2-f2e1-48af-b377-e693834cce7a)

如上图所示，在基于共识协议的副本机制中，每个副本具有**命令日志**和**状态机**。</br>
其中命令日志记录着用户发出的请求，如键值对更新;状态机按顺序执行命令日志中的请求。只要每个副本的命令日志完全相同，则每个副本的状态机也会完全一致。共识协议用来保证不同副本的命令日志内容相同。

#### 2.分布式二级索引技术

分布式键值存储系统使用二级索引支持从二级键访问键值数据。二级索引维护从二级键到主键的映射，可以像主索引一样被分散至多台存储服务器，以提高扩展性。

![image](https://github.com/lus-oa/Distributed-Storage/assets/122666739/963ee308-5611-42d6-a687-1ab5752d0a1e)

- 在第一种方式中，二级键与对应的主键存在同一台服务器，如图 (a)所示。在这种方式中，由于一个二级键会对应多个主键(二级键“1997”对应的主键存在于两台服务器)所以对于二级索引查询操作，客户端需要并行地发送请求至所有的服务器，浪费大量网络资源;但对于主键修改操作，该方式只涉及一台服务器，延迟低目不需要分布式协调。

![image](https://github.com/lus-oa/Distributed-Storage/assets/122666739/d37a56a9-97e6-4025-8d6b-c548438032dd)

- 在第二种方式中，二级索引根据二级键被分散到多台机器，如图(b)所示。

在这种方式中，二级索引查询操作需要两次网络往返:首先查询对应二级索引所在的服务器，获得对应的主键列表;然后根据这些主键查询对应的主索引所在的服务器，获得目标键值对数据。主索引修改需要多次网络交互:首先发送到主索引对应的服务器(记作 A),查询到旧的二级键;然后A 发送数据至新的二级键所在的二级索引服务器，请求建立新的二级索引条目;接着A将新的键值对写入主索引;最后，A发送数据至旧的二级键所在的二级索引服务器，请求删除旧的二级索引条目。

## 分布式对象存储系统

分布式对象存储系统将对象存储在大的存储服务器中，提供对象访问接口。分布式对象存储系统允许用户创建存储多个对象的桶(bucket)。这个对象包括三个部分：Key、Data、Metadata 。每个对象大小不等，最高可达数 TB，由全局唯一ID来识别。分布式对象存储系统提供对对象的读/写操作，还提供对对象内某段范围的访问。<br/>
分布式对象存储系统除了拥有云存储的传统优势(如现收现付、规模经济)外，还向用户提供可单独扩展的计算和存储资源。分布式对象存储系统目前常用于存储非结构化数据，即没有预定义数据模型的数据，比如办公文档、图片音频、视频等。

![image](https://github.com/lus-oa/Distributed-Storage/assets/122666739/5a3478a5-9cb0-4fa2-9776-fba47dc24be9)
